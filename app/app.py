...
if page == "📊 Overview":
    ...
    with col_r:
        ...
        events = df_raw["key_events"].dropna()
    ...
    # ── Bottom row: 4 sparkline-style charts ─────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.caption("Select up to 4 indicators to compare side-by-side on a normalised (z-score) axis.")
    ...
    if selected:
        ...
        fig.add_annotation(x=imf_years[0] if imf_years else 2009,
                           y=df_plot.max().max() * 0.9,
    ...
    df_ext = df_filtered[["exports_usd_bn", "imports_usd_bn",
                           "remittances_usd_bn", "trade_balance_usd_bn"]].copy()
    fig2 = go.Figure()
    for col, color in [("exports_usd_bn", "#43A047"),
                        ("imports_usd_bn", "#E53935"),
                        ("remittances_usd_bn", "#1E88E5")]:
        fig2.add_trace(go.Scatter(x=df_ext.index, y=df_ext[col],
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    col_a = st.selectbox("X-axis feature", numeric_cols, index=numeric_cols.index("remittances_usd_bn"))
    col_b = st.selectbox("Y-axis feature", numeric_cols, index=numeric_cols.index("gdp_growth_pct"))
    ...
    with c2:
        ...
        x_, y_ = df_filtered[[col_a, col_b]].dropna().values.T
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if model is None:
        st.warning(
            ...
            "Run the notebook `notebooks/01_EDA.ipynb` end-to-end first, "
            "then refresh this page."
        )
    else:
        # Back-test: predict on full feature matrix
        ...
        # Shade test period (last 5 years)
        ...
        fig.update_layout(title="GDP Growth: Actual vs Model Predictions (Full Period)",
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if model is None:
        st.warning("Train the model first by running the notebook.")
    else:
        # Base: use last available year's feature values
        ...
        predicted_growth = model.predict(X_scenario)[0]
        ...
        with col_result:
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                delta={"reference": float(df_raw.loc[2025, "gdp_growth_pct"])},
                gauge={
                    "axis": {"range": [-3, 10]},
                    "bar":  {"color": colour},
                    "steps": [
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    **Technical Stack**
    ...
    | Feature engineering | Custom domain features (lag, rolling, ratios) |
    | Models             | Ridge, Lasso, RandomForest, GradientBoosting |
    | Validation         | TimeSeriesSplit cross-validation (no leakage) |
    | Explainability     | Feature importance, SHAP |
    | Dashboard          | Streamlit + Plotly |
    | Reproducibility    | joblib, JSON manifests, requirements.txt |
    ...
    **How to Run**
    ...
    jupyter lab notebooks/01_EDA.ipynb  # Run all cells to train model
