...
from src.features.build_features import (
    ...
    add_pkr_depreciation,
    add_forex_cover,
    add_trade_openness,
    add_external_pressure_index,
    build_all_features,
)
...
def base_df():
    """Synthetic 15-year dataset for feature tests."""
    np.random.seed(0)
    n = 15
    return pd.DataFrame(
        {
...
class TestLagFeatures:
    def test_lag1_created(self, base_df):
        ...
        assert "gdp_growth_pct_lag1" in result.columns

    def test_lag1_values_shifted(self, base_df):
        result = add_lag_features(base_df, "gdp_growth_pct", [1])
        # First value of lag1 should be NaN
        assert pd.isna(result["gdp_growth_pct_lag1"].iloc[0])
        # Second value should equal first original
        assert result["gdp_growth_pct_lag1"].iloc[1] == base_df["gdp_growth_pct"].iloc[0]

    def test_multiple_lags(self, base_df):
        result = add_lag_features(base_df, "gdp_growth_pct", [1, 2, 3])
        for lag in [1, 2, 3]:
            assert f"gdp_growth_pct_lag{lag}" in result.columns
...
class TestRollingFeatures:
    def test_rolling_mean_created(self, base_df):
        ...
        assert "gdp_growth_pct_ma3" in result.columns

    def test_rolling_mean_close_to_average(self, base_df):
        result = add_rolling_features(base_df, "gdp_growth_pct", [3])
        # At index 4: mean of rows 2, 3, 4 (window=3, min_periods=1)
        expected = base_df["gdp_growth_pct"].iloc[2:5].mean()
        actual = result["gdp_growth_pct_ma3"].iloc[4]
...
class TestForexCover:
    ...
    def test_formula_correct(self, base_df):
        result = add_forex_cover(base_df)
        expected = (base_df["forex_reserves_usd_bn"] / base_df["imports_usd_bn"]) * 12
        pd.testing.assert_series_equal(
            result["forex_months_import"],
            expected,
            check_names=False,
            check_exact=False,
            rtol=1e-9,
...
class TestBuildAllFeatures:
    ...
    def test_expected_engineered_cols(self, base_df):
        result = build_all_features(base_df)
        # Column names follow the pattern <source_col>_lag<n> and <source_col>_ma<w>
        expected = [
            "gdp_growth_pct_lag1",
            "gdp_growth_pct_lag2",
    ...
    def test_no_nans_after_build(self, base_df):
        result = build_all_features(base_df)
        # build_all_features calls df.dropna() at the end
